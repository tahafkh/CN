...
ofstream logger = ofstream("./logs/router.log");
...
double get_avg_queue_len(int q_len) {
    double avg;
...
void calc_pd_pa(double &pd, double &pa, int avg) {
    ...
    if (count == 1 / maxp) {
        ...
        logger << "Count has reached 1/maxp. Dropping packet" << endl;
        pa = 1.0;
    }
}

bool red_check_queue() {
    int q_len = buffer_size;
    double new_avg = get_avg_queue_len(q_len);
...
void send_packet() {
    ...
    while (true) {
        ...
        if (front_frame.data[IS_ACK_INDEX] != 0x0) {  // Is ACK
            ...
            logger << "Sending ACK " << seq_num << " to " << ntohs(forwarding_table[dest_port].sin_port) << endl;
        }
        else
            logger << "Sending packet " << seq_num << " to " << ntohs(forwarding_table[dest_port].sin_port) << endl;
...
void read_frame() {
    ...
    if (rand()%100 < LOSS_RATE) {
        logger << "packet lost" << endl;
        buffer_mutex.unlock();
        return;
    }
    
    if (red_enabled && !red_check_queue()) {
        logger << "packet dropped" << endl;
        buffer_mutex.unlock();
        return;
    }
    ...
    logger << "Received frame " << seq_num << " from " << ntohs(station_addr.sin_port) << endl;
...
void recv_packet() {
    logger << "Router is listening..." << endl;
    srand(time(nullptr));
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 3) {
        ...
        red_enabled = atoi(argv[2]);
    } else {
        logger << "usage: router <buffer_size> <red_enable>" << endl;
        return 1;
    }

    if (!setup_connection()) {
        logger << "router setup failed" << endl;
        return 1;
    }
    ...
    logger << "Finishing up send and receive" << endl;
    logger << "Done" << endl;
    sleep(3000);
